...
private val MOBILE_REGEX = Regex("(\\+\\d+)?1[34578]\\d{9}$")
private val PHONE_REGEX = Regex("(\\+\\d+)?(\\d{3,4}\\-?)?\\d{7,8}$")
 ...
 * ```
 */
fun String.isMobile(): Boolean = MOBILE_REGEX.matches(this)
 ...
 * ```
 */
fun String.isPhone(): Boolean = PHONE_REGEX.matches(this)
 ...
 * Validates if a string matches an email address pattern.
 */
fun String.isEmail(): Boolean = EMAIL_REGEX.matches(this)
