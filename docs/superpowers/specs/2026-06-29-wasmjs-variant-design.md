...
| V5 核心 hooks wasmJs 功能正确 | ✅ 完整 | **`wasmJsBrowserTest` 全绿：201 tests, 0 failures, 0 ignored**（含 `WasmJsSmokeTest` 4 项：createStore/reducer/combineStores/middleware，验证 KClass 作 Map 键在 wasmJs 运行时可用；以及继承自 commonTest 的 Fetch/Redux/Debounce/Form 等全部 197 项）。在 headless Chrome（`useChromeHeadlessNoSandbox`）中通过 karma+webpack 端到端执行 |
...
### 10.5 浏览器测试的完整端到端验证（2026-06-29 补充）

V5 已**完整验证**，非"部分"：

- **关键配置**：`wasmJs { browser { testTask { useKarma { useChromeHeadlessNoSandbox() } } } }`。普通 `ChromeHeadless` 在无头/CI 环境无法启动（无 sandbox），`useChromeHeadlessNoSandbox()` 是 KMP 内置的 launcher，自动加 `--no-sandbox` / `--disable-dev-shm-usage`。
- **环境前置**：karma tooling 的 npm 依赖（webpack/karma/mocha 等 376 包，含 git 依赖 `karma: Kotlin/karma#6.4.5`）首次安装较慢（~3min），需在网络可用环境执行一次 `kotlinWasmToolingSetup`。
- **结果**：`./gradlew :hooks:wasmJsBrowserTest` → **BUILD SUCCESSFUL，201 tests, 0 failures, 0 ignored**，在真实 headless Chrome 中经 karma+webpack 端到端执行。
- **验证范围**：继承自 commonTest 的 197 项（Fetch/Redux/Debounce/Form/Table 等）+ wasmJs 专属 `WasmJsSmokeTest` 4 项。证明 §3.1 核心 hooks 在 wasm 运行时功能正确。
